...
class avaris_trans(models.Model):
    _name = "avaris.claim"
...
class CrmClaim(models.Model):
    ...
    @api.model
    def _selection_model2(self):
        return [
            (x, _(self.env[x]._description)) for x in APPLICABLE_MODELS_recla if x in self.env
        ]
    ...
    model_ref_id2 = fields.Reference(
        selection="_selection_model2", string="Type de Réclamation"
    )

    
    invoice = fields.Many2one(comodel_name="account.move", string="N°Facture: ")
    order = fields.Many2one(comodel_name="sale.order", string="N°commande: ")
    livraison = fields.Many2one(comodel_name="stock.picking", string="N°Bon de Livraison: ")
    ...
    def onchange_invoice_id(self):
        for rec in self:
            rec.invoice_line_ids = rec.invoice.invoice_line_ids
            rec.partner_id = rec.invoice.partner_id.id
            rec.onchange_partner_id()

            if rec.invoice.invoice_line_ids:
                if rec.invoice.invoice_line_ids[0].sale_line_ids:
                    rec.order = rec.invoice.invoice_line_ids[0].sale_line_ids[0].order_id.id
                    if rec.order.picking_ids:
                       rec.livraison = rec.order.picking_ids[0].id
...
class AccountMoveLine(models.Model):
    ...
    quantity_claim = fields.Float(
        string='Quantity claim',
        store=True, readonly=False,
        digits='Product Unit of Measure',
        help="The quantity claim eg: number of product impacted by this claim"
             "",
    )

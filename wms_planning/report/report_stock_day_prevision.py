...
class ReportStockDayPrevision(models.Model):
    _name = 'report.stock.dayprevision'
    _auto = False
    _description = 'Stock Quantity Report by day'
...
SELECT
    COALESCE(warehouse_id, 0)::int * 10000000 + COALESCE(product_id, 0)::int * 100 + (date::date - CURRENT_DATE)::int AS id,
...
FROM (   
    ...
    FROM
        ...
        date_trunc('day',(now() at time zone 'utc')::date + interval 'futur_horizon')::date, '1 day'::interval) date,
        stock_quant q
...
UNION ALL 
    ...
    SELECT
        ...
        GENERATE_SERIES(date_trunc('day',
            ...
            date_trunc('day', (now() at time zone 'utc')::date + interval 'futur_horizon'),

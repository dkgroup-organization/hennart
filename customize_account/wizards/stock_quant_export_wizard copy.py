...
class StockQuantExportWizard(models.TransientModel):
    ...
    def _get_parent_lot_from_mo(self, finished_lot):

        """Retourne un lot 'amont' (composant) si le lot fini provient d'un OF."""
        Production = self.env['mrp.production']
        # On cherche un OF qui a produit ce lot (via les lignes de mouvements finis)
        mo = Production.search([('finished_move_line_ids.lot_id', '=', finished_lot.id)], order='id desc', limit=1)
        if not mo:
            return False

        # Lots composants réellement consommés sur l’OF
        parent_lots = mo.move_raw_ids.move_line_ids.mapped('lot_id')
        parent_lots = parent_lots.filtered(lambda l: l)  # enlève les False

        if not parent_lots:
            return False

        # Heuristique : 
        # 1) si un seul lot composant → on prend celui-là
        # 2) sinon, on essaie le lot composant ayant le plus grand unit_weight
        # 3) sinon, on prend le premier
        if len(parent_lots) == 1:
            return parent_lots[0]

        # lot avec le plus grand poids déclaré
        weighted = parent_lots.filtered(lambda l: l.unit_weight and l.unit_weight > 0.0)
        if weighted:
    ...
    def action_export(self):
        ...
        headers = [
            'Code', 'Produit', 'Lot', 'Valorisation manuelle du stock', 'Unité vente', 'Unité achat', 'Quantité composant','Poids (moyen du lot à récpetion)', 'Quantité en stock',
            'Fournisseur', 'Date entrée', 'Prix unitaire', 'Cout transport','Cout transformation','Cout découpe','Cout affinage','Prix de revient'
        ]

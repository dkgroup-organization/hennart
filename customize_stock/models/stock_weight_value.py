...
class StockWeightValue(models.Model):
    ...
    name = fields.Char('Weighing number', select=True)
    date = fields.Datetime('date', default=lambda self: fields.Datetime.now())
    device_id = fields.Many2one('stock.weight.device', 'Weight Device', select=True)
    ...
    picking_id = fields.Many2one('stock.picking', 'Reference', select=True)
